...
function HomeInner() {
  ...
  // -----------------------------------------------------------------------
  useEffect(() => {
    // Kick off the listings fetch IMMEDIATELY (in parallel with the auth
    // round-trip). The /api/listings/search endpoint doesn't depend on the
    // user identity for anonymous browsing — gating listings behind the
    // ~200-500ms auth lookup made the logged-in home 4-6x slower to first
    // paint than the anon home, since auth-cookie verification was on the
    // critical path of the listings query. By firing both in parallel,
    // listings paint as soon as the API responds regardless of whether
    // auth has resolved yet.
    if (!hasInitialViewportLoad.current) {
      hasInitialViewportLoad.current = true;
      // Default NYC bounds — the map onBoundsChange will override later.
      void loadForViewport({ latMin: 40.65, latMax: 40.82, lonMin: -74.05, lonMax: -73.88 });
    }
